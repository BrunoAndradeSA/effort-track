...
function commitPendingEmail(badgeContainer) {
  const inner = badgeContainer.querySelector(".badge-input-inner");
  const input = inner.querySelector(".badge-text-input");
  const raw = input.value.trim();
  if (!raw) return;
  createBadgeChip(raw, inner, input);
  input.value = "";
}
...
export function initSettingsUi() {
  ...
  const inner = badgeContainer.querySelector(".badge-input-inner");
  const badgeInput = inner.querySelector(".badge-text-input");
  ...
  badgeInput.addEventListener("input", () => {
    ...
    parts.forEach(p => {
      ...
      if (trimmed) createBadgeChip(trimmed, inner, badgeInput);
    });
  ...
  badgeInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      commitPendingEmail(badgeContainer);
    }
    if (e.key === "Backspace" && badgeInput.value === "") {
      const chips = inner.querySelectorAll(".badge-chip");
  ...
  function renderEmails(emails) {
    inner.querySelectorAll(".badge-chip").forEach(el => el.remove());
    emails.forEach(email => createBadgeChip(email, inner, badgeInput));
  }
