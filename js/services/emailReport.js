...
export function generatePlainTextBody(project, metrics) {
  ...
  const lines = [
    ...
    ``,
    `── Produção Realizada ──`,
    `Horas DEV realizadas: ${project.hoursDevRealized}h de ${project.estimatedDevHours}h (${Math.round(metrics.devProgressReal)}%)`,
    `Horas QA realizadas:  ${project.hoursQaRealized}h de ${project.estimatedQaHours}h (${Math.round(metrics.qaProgressReal)}%)`,
